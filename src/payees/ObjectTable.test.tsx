...
import { ColumnConfig, Payee } from './payees-types';

describe('PayeesListRow', () => {
  let columns: ColumnConfig<Payee>[], payees: Payee[], thead: HTMLTableSectionElement;
  ...
  test('loads and renders a row', () => {
    const { queryByText } = render(
      <ObjectTableRow columns={columns} object={payees[0]} />,
      {
  ...
  test('runs an event handler', () => {
    ...
    const { queryByText } = render(
      <ObjectTableRow  columns={columns} object={payees[0]} selectObject={mockHandler} />,
      {
        container: document.body.appendChild(thead),
      },
    );
...
function getColumnConfig(): ColumnConfig<Payee>[] {
  return [
    {
      field: 'payeeName',
      label: 'Payee Name',
